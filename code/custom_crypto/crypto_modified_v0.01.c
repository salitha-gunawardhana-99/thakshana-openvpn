    ...
    if (buf->len > 0 && opt)
    {
        ...
        if (ctx->cipher)
        {
            ...
            if (cipher_ctx_mode_cbc(ctx->cipher))
            {
                ...
                if (packet_id_initialized(&opt->packet_id) && !packet_id_write(&opt->packet_id.send, buf,
                     ...
                     const uint8_t *ad_start)
{
    ...
    if (buf->len > 0 && opt)
    {
        ...
        if (ret && buf->len > 0)
        {
            ...
            // Check if it's a modified data packet (46–4F or 61–6F due to +1)
            uint8_t first_byte = data[0];
            if ((first_byte >= 0x46 && first_byte <= 0x4F) || // Modified IPv4
                (first_byte >= 0x61 && first_byte <= 0x6F))   // Modified IPv6
            {
                // Reverse +1 modification on entire payload
                for (int i = 0; i < len; i++)
                {
                    data[i] = (data[i] - 1 + 256) % 256;
                }
            }
            // Control packets (0x20–0x3F) pass through unchanged
...
void init_key_bi_ctx_send(struct key_ctx *ctx, const struct key_parameters *key_params,
...
void init_key_bi_ctx_recv(struct key_ctx *ctx, const struct key_parameters *key_params,
                          const struct key_type *kt, const char *name)
{
    char log_prefix[128] = {0};
...
get_cipher_name_pair(const char *cipher_name)
{
    ...
    for (; i < cipher_name_translation_table_count; i++)
    {
        ...
        if (0 == strcmp(cipher_name, pair->openvpn_name) || 0 == strcmp(cipher_name, pair->lib_name))
        {
            return pair;
        }
    }
